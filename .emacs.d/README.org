...
#+begin_src emacs-lisp :tangle yes
(setq
 org-agenda-files (list "~/local_tasklist/local_tasklist.org"
                        "~/local_tasklist/local_tasklist_longterm.org")
 org-agenda-tag-filter-preset '("-archived" "-noagenda"))

(setq org-agenda-custom-commands '(("q" "Florent's Custom View"
...
(use-package git-auto-commit-mode
  :hook (org-mode . (lambda ()
                      (when (string-match "/tasklist/" buffer-file-name)
...
(use-package lsp-mode
  ...
  (setq lsp-dart-sdk-dir "/usr/local/Cellar/dart/2.7.2/libexec/")
  :hook (
  (go-mode . lsp)
  (dart-mode . lsp)
  )
      ...
      (let ((packages (cl-remove-if (lambda (s) (s-contains? "/vendor/" s))
                                    (s-split "\n"
                                             (shell-command-to-string (el-patch-swap
                                                                        ...
                                                                        "go list ./..."
                                                                        ;; Replacement
                                                                        (format "go list %s/..." (projectile-project-root))
                                                                        ))))))
